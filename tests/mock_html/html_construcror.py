...
HTML_BASE = (
    '<!DOCTYPE html><html lang="ru-RU"><head><meta charset="utf-8"/></head><body '
...
POSTERS = (
    '<div class="b-content__inline"><div class="b-content__inline_inner b-content__inline_inner_mainprobar '
...
SAMPLE_POSTERS = (
    '<div class="b-content__inline_item" data-id="{id}" data-url="{url}"><div class='
...
SAMPLE_COLLECTIONS = (
    '<div class="b-content__collections_item" data-url="{url}"><img class="cover" src="{img_url}" '
...
SAMPLE_COMMENT_ITEM = (
    '<li id="comments-tree-item-{id}" class="comments-tree-item" data-id="{id}" '
...
SPOILER = (
    '<!--dle_spoiler--><div class="title_spoiler"><img id="image-sp0123456789abcdef0123456789abcdef" '
...
EDITED = (
    ' <span class="edited hd-tooltip" title="Отредактировано Администрацией"></span>'
)
REPORT = (
    '<i title="Пожаловаться на комментарий" class="b-comment__report hd-tooltip" '
    'data-id="{id}"></i>\r\n            '
)
LINK = '<a href="{text}" target="_blank">{text}</a>'
YOUTUBE_URL = (
    '<!--noindex--><a class="youtu-link" data-youtube-id="{url_id}"  '
    'href="{url}" rel="nofollow" target="_blank">смотреть</a><!--/noindex-->'
)
TAG = ' <a class="tag-link" href="javascript:void(0)" rel="nofollow">{text}</a>'

SAMPLE_NAVIGATION = (
    '<!--noindex--><div class="b-navigation">{data}</div><!--/noindex-->'
)
SAMPLE_NAVIGATION_ITEM = (
    '<a rel="nofollow" onclick="sof.comments.loadComments({id}, {page}, false, 0);'
...
SAMPLE_TRAILER = (
    '<iframe width="640" height="360" src="{trailer_url}" frameborder="0" '
...
def generate_poster_html(content: List[Dict[str, Any]]) -> str:
    ...
    for item in content:
        info = re.sub(
            r"(\d+ сезон) (\d+ серия)",
            r"\1, \2",
            item["status"] if item["status"] else "",
        )
    ...
    return HTML_BASE.format(
        type=POSTERS.format(data=data).replace("    ", "").replace("\n", "")
    )
...
def generate_collections_html(content: List[Dict[str, Any]]) -> str:
    ...
    return HTML_BASE.format(
        type=COLLECTIONS.format(data=data).replace("    ", "").replace("\n", "")
    )
...
def generate_fake_html(reference_name: str) -> Tuple[List[Dict[str, Any]], str]:
    ...
    if reference_name == "collections":
        return copy.deepcopy(reference_data)[reference_name], generate_collections_html(
            reference_data[reference_name]
        )
    return copy.deepcopy(reference_data[reference_name]), generate_poster_html(
        reference_data[reference_name]
    )
...
def convert_datetime_into_string(datetime_string):
    ...
    return timestamp.strftime("%d {month} %Y %H:%M").format(
        month=month_name[timestamp.month]
    )


def prepare_comment_text(comment_text: str) -> str:
    def convert(pattern, new_pattern, text, fun=None) -> str:
        while re.search(pattern, text, flags=re.S):
            match = re.search(pattern, text, flags=re.S)
            formatted_text = new_pattern.format(
                **(fun if fun is not None else lambda x: {"text": x.group(1)})(match)
            )
            text = re.sub(pattern, formatted_text, text, count=1, flags=re.S)
        return text

    comment_text = convert(
        r'(https?://youtu[^\s<>"\(\)\,]+(?![^<]*</a>))',
        YOUTUBE_URL,
        comment_text,
    ...
    comment_text = convert(
        r'(https?://rezka\.ag[^\s<>"\(\)\,]+(?![^<]*</a>))', LINK, comment_text
    )
    comment_text = convert(r"(?:^|\s)(#[A-z0-9]+(?![^<]*</a>))", TAG, comment_text)
    comment_text = convert(
        r"<spoiler>((?:(?!<\/?spoiler>).)*)<\/spoiler>", SPOILER, comment_text
    )
...
def generate_comment(comments: List[Dict[str, Any]], indentation=0) -> str:
    ...
    for comment in comments:
        comment_content = {
            ...
            "alt_name": comment["author"]["name"]
            if comment["author"]["img_url"] != NO_AVATAR
            else "",
            "user_name": comment["author"]["name"],
            "timestamp": "оставлен "
            + convert_datetime_into_string(comment["timestamp"]),
            ...
            "replies": generate_comment(
                comment["replies"], indentation=indentation + 1
            ),
            "style": ' style="display: inline;"' if comment["likes_num"] > 0 else "",
            "class": ' class="positive"' if comment["likes_num"] > 0 else "",
            "admin": " b-comment__admin"
            if comment["author"]["name"] == "Администрация"
            else "",
            "report": REPORT.format(id=comment["id"])
            if comment["author"]["name"] != "Администрация"
            else "",
        }
...
def generate_navigation_string(
    film_id: Union[str, int], current_page: int, first_page: int, last_page: int
) -> str:
    result_string = ""
    ...
    if current_page != first_page:
        result_string += SAMPLE_NAVIGATION_ITEM.format(
            id=film_id, page=current_page - 1, view=SAMPLE_PREVIEW_NAVIGATION_ITEM
        )
    ...
    if current_page - 6 < first_page:
        for page in range(first_page, last_page + 1)[:10]:
            if page != current_page:
                result_string += SAMPLE_NAVIGATION_ITEM.format(
                    id=film_id, page=page, view=page
                )
            else:
        ...
        if last_page > 10:
            ...
            result_string += SAMPLE_NAVIGATION_ITEM.format(
                id=film_id, page=last_page, view=last_page
            )
    elif current_page - 5 > first_page and current_page + 4 < last_page:
        result_string += SAMPLE_NAVIGATION_ITEM.format(
            id=film_id, page=first_page, view=first_page
        )
        result_string += SAMPLE_INTERMEDIATE_NAVIGATION_ITEM
    ...
    if current_page != last_page:
        result_string += SAMPLE_NAVIGATION_ITEM.format(
            id=film_id, page=current_page + 1, view=SAMPLE_NEXT_NAVIGATION_ITEM
        )

    return SAMPLE_NAVIGATION.format(data=result_string)


def generate_comments_tree() -> List[
    Tuple[List[Dict[str, Any]], Dict[str, Union[str, int]]]
]:
    ...
    for film_id in reference_data:
        for page in reference_data[film_id]:
            response_obj = {
                "navigation": generate_navigation_string(
                    film_id, int(page), 1, len(reference_data[film_id])
                ),
...
def generate_trailer_info() -> List[
    Tuple[Dict[str, Optional[Union[str, int]]], Dict[str, Union[bool, str]]]
]:
