...
class BaseMovie:
    _metadata: Union[MovieQueryData, SerialQueryData]
    ...
    def get_video_url(self, quality: Optional[Quality] = None) -> Union[Dict[str, List[str]], List[str]]:
    ...
    def set_translate(self, translate: Union[Translator, int, str], is_director: Optional[bool] = False):
        if isinstance(translate, Translator):
            if translate in self.translate_list:
                return self._set_translate(translate)
            raise ValueError(
        ...
        if isinstance(translate, int) and not isinstance(translate, bool):
            ...
            if len(translates_list) == 1:
                return self._set_translate(translates_list[0])
            if len(translates_list) > 1:
                ...
                return self._set_translate(correct_translate[0])
            raise ValueError(
                f"This 'translate' attribute ({translate}) is not in "
                f"this translator IDs list {[t.id for t in self.translate_list]}."
            )
        if isinstance(translate, str):
            translates_list = [i for i in self.translate_list if i.title == translate]
            if len(translates_list) == 1:
                return self._set_translate(translates_list[0])
            raise ValueError(
                f"This 'translate' attribute ({translate}) is not in "
    ...
    def _set_translate(self, translate: Translator):
    ...
    def load(
            self,
            file_name: str,
            quality: Quality = Quality.Q1080p,
            subtitle: Optional[str] = None,
            create_dump_file: bool = False,
            chunk_size: int = 2 ** 10 * 512,
    ):
        media_loader.load_from_player(self, f"{file_name}.mp4", quality, create_dump_file, chunk_size)
...
class Film(BaseMovie):
    ...
    def __repr__(self):
        return f'<{Film.__name__}(id="{self._metadata.id}")>'


class Serial(BaseMovie):
    _metadata: SerialQueryData
    ...
    def set_season(self, season_id: Optional[int] = None, *, season_title: Optional[str] = None) -> Serial:
    ...
    def set_episode(self, episode_id: Optional[int], *, episode_title: Optional[str] = None):
        ...
        target_season = [s for s in self.seasons_tabs if s.id == self._metadata.season]
        target_episode = [e for e in target_season[0].episodes if e.id == episode_id or e.title == episode_title]

        if len(target_episode) == 0:
            if episode_title is not None:
                raise ValueError(
                    f"This 'episode_title' attribute ({episode_title}) is missing "
                    f"from episode list {[e.title for e in target_season[0].episodes]}"
                )
            raise ValueError(
                f"This 'episode_id' attribute ({episode_id}) is missing "
                f"from episode IDs list {[e.id for e in target_season[0].episodes]}"
            )
    ...
    def set_translate(self, translate: Union[Translator, int, str]):
        try:
            ...
            return super().set_translate(translate=translate, is_director=None)
        finally:
    ...
    def set_params(
            ...
            *,
            ...
            translate: Union[Translator, int, None] = None,
        ...
        if translate is not None:
            self.set_translate(translate=translate, is_director=is_director)
    ...
    def __repr__(self):
        return f'<{Serial.__name__}(id="{self._metadata.id}")>'
...
class PlayerBuilder(PageRepresentation):
    ...
    @staticmethod
    def make_seasons_list(seasons_string: str):
